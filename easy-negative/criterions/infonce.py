...
class InfoNCELoss(nn.Module):
    ...
    def forward(self, prediction_embs, target_embs, choices_dim=1, output_head=None, target_head=None):
        ...
        if len(target_embs.shape) != 3:
            sz = prediction_embs.shape # [bs,128]
            prediction_embs = prediction_embs.view(-1, sz[-2], sz[-1]) #[1,bs,128]
            target_embs = target_embs.view(-1, sz[-2], sz[-1]) # [1,bs,128]

        # Compute scores
        scores = torch.bmm(prediction_embs, target_embs.transpose(-2, -1)) / self.temperature # [1,bs,bs]
        scores = scores.flatten(0, 1) # [bs,bs]

        # Labels
        bs, n_choices = target_embs.shape[:2] # [bs=1,n_choices = actual batch size]

...
sys.path.insert(0,'../')
from data_loader_test import Vgg_Sound
import models
import torch
...
# python test_batch.py config.yaml --model_type 'audio' --lin_prob True
...
tt_Dataset = Vgg_Sound(cfg,model_type = args.model_type)
...
print('model type is', args.model_type, 'linear prob is ', args.lin_prob,'test accuracy is',acc)
top5_least5_classes= [181,133,210,200,194,69,1,45,308,270]
#least5_classes= [69,1,45,308,270]
for i in top5_least5_classes:
  inx = np.where(y_true==i)[0]
  y_true_ = y_true[inx]
  y_pred_ = y_pred[inx]
  acc = accuracy_score(y_true_, y_pred_)
  acc = np.round(acc,decimals = 3)
  print('class is', i, tt_Dataset.classes[i], 'accuracy is', acc)
